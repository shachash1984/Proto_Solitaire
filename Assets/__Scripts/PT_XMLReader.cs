	...
	*/
    ...
    string ParseTag(string eS, PT_XMLHashtable eH)
    {
        ...
        if (ndx == -1)
        {
            ...
            return (""); // We're done with this tag //***if it's just text he returns an empty string but saving the text to the hashtable text property
        }
        ...
        if (eS[ndx + 1] == '?')
        {
            ...
            eH.header = header;
            return (eS.Substring(ndx2 + 2)); //***saving the header to the header propery of the hashlist and returning only whats after the header
        }
        ...
        if (eS[ndx + 1] == '!')
        {
            ...
            if (SHOW_COMMENTS) Debug.Log("XMl Comment: " + comment); //showing comments without saving them (can be turned off)
                                                                     //eH["@XML_Header"] = header;
        ...
        end = Mathf.Min(end1, end2, end3); //***if all chars above exist end = the closest one
        string tag = eS.Substring(ndx + 1, end - ndx - 1); //*** tag = all the chars between the char after the  "<" and the closest char from the above
        ...
        string atts = "";
        if (end1 < end3)
        { //**only if " " is the closest char to '<' then atts is assigned " ....>"
            try
            {
        ...
        while (atts.Length > 0)
        {
            ...
            att = atts.Substring(0, eqNdx); //**att is assigned ".....="
