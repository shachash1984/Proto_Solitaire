    ...
    public static Vector3 BoundsInBoundsCheck(Bounds bigB, Bounds lilB, BoundsTest test = BoundsTest.onScreen)
    {
        ...
        switch (test)
        {
            ...
            case BoundsTest.center:
                ...
                if (bigB.Contains(lilB.center))
                {
                    return (Vector3.zero);
                }
                // if not contained, find the offset
                if (lilB.center.x > bigB.max.x)
                {
                    off.x = lilB.center.x - bigB.max.x;
                }
                else if (lilB.center.x < bigB.min.x)
                {
                    off.x = lilB.center.x - bigB.min.x;
                }
                if (lilB.center.y > bigB.max.y)
                {
                    off.y = lilB.center.y - bigB.max.y;
                }
                else if (lilB.center.y < bigB.min.y)
                {
                    off.y = lilB.center.y - bigB.min.y;
                }
                if (lilB.center.z > bigB.max.z)
                {
                    off.z = lilB.center.z - bigB.max.z;
                }
                else if (lilB.center.z < bigB.min.z)
                {
                    off.z = lilB.center.z - bigB.min.z;
                }
                return (off);
